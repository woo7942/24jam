...
import { ArrowLeft, MapPin, Calendar, Clock, Package, Loader2, Inbox, CheckCircle2, Phone, Sparkles } from "lucide-react";
...
export default function DriverRequestsPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    // 1. 매칭된 요청 (selected 상태)
    const { data: matchedData, error: matchedError } = await supabase.rpc("get_my_matched_requests");
    if (matchedError) {
    ...
    const { data: myBids } = await supabase.from("bids").select("request_id").eq("driver_id", user.id);
    const myBidIds = new Set((myBids || []).map((b) => b.request_id));
    const requestIds = (requestsData || []).map((r) => r.id);

    let bidCounts: Record<string, number> = {};
    if (requestIds.length) {
      const { data: allBids } = await supabase.from("bids").select("request_id").in("request_id", requestIds);
      bidCounts = (allBids || []).reduce((a: Record<string, number>, b) => {
        ...
        {matched.length > 0 && (
          <section>
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="font-bold text-base">매칭 완료된 요청 ({matched.length}건)</h2>
            </div>
            <div className="space-y-3">
              {matched.map((m) => (
                <div key={m.bid_id} className="rounded-2xl border-2 border-mint-300 bg-mint-50/50 p-4">
                  <div className="flex items-center gap-1 mb-2">
                    <CheckCircle2 className="h-4 w-4 text-mint-600" />
                    <span className="text-xs font-semibold text-mint-700">매칭 완료</span>
                  ...
                  <div className="space-y-1 text-sm text-gray-700 mb-3">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span>{TIME_SLOT_LABELS[m.time_slot] || m.time_slot}</span>
                    </div>
                    {m.box_count > 0 && (
                      <div className="flex items-center gap-1.5">
                  ...
                  <div className="rounded-xl bg-white border p-3">
                    <div className="text-[11px] font-semibold text-gray-500 mb-1">고객 정보</div>
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="font-bold text-sm">{m.customer_name}</div>
                        <div className="text-xs text-gray-600">{m.customer_phone}</div>
                      </div>
                      <a
                  ...
                  {m.notes && (
                    <div className="mt-2 text-xs text-gray-600 bg-white rounded-lg p-2 border">
                      💬 {m.notes}
                    </div>
                  )}
                </div>
        ...
        <section>
          ...
          {requests.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-xs text-gray-500 mt-1">새 요청이 들어오면 알려드릴게요</p>
            </div>
          ) : (
            <div className="space-y-3">
              <p className="text-xs text-gray-500 mb-2">총 {requests.length}건의 요청</p>
              {requests.map((req) => {
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[11px] font-medium text-gray-500">{remaining}</span>
                      </div>
                      <span className="text-[11px] text-gray-500">입찰 {req.bid_count ?? 0}건</span>
                    </div>
